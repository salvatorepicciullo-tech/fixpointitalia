...
export default function FixpointValuations() {
  ...
  const load = async () => {
    ...
    const r = await fetch(`${API}/api/fixpoint/valuations`, {
      headers: {
...
if (Array.isArray(data)) {
 ...
 const changeStatus = async (id: number, status: string) => {
  ...
  await fetch(`${API}/api/fixpoint/valuations/${id}/status`, {
    method: 'PUT',
    headers: {
  ...
  <div>
    📅{" "}
    {v.preferred_datetime.split("T")[0].split("-").reverse().join("/")} — ⏰{" "}
    {v.preferred_datetime.split("T")[1].substring(0, 5)}
  </div>
)}
