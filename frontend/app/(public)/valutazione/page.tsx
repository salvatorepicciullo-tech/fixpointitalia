        ...
        >
          ...
          <div className="flex justify-center gap-4 text-sm">
            {['Dispositivo','Marca','Modello','Dettagli'].map((label,i)=>{
              ...
              const active=step===s;
              return(
                <div key={i} className={`flex items-center gap-2 ${active?'text-blue-600':'text-gray-400'}`}>
                  <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${active?'bg-blue-600 text-white':'border border-gray-300'}`}>
                    {s}
                  </span>
                  {label}
                </div>
