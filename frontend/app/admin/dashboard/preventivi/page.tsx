...
const load = async () => {
  try {
    const [qRes, fRes] = await Promise.all([
      fetch(`${API_URL}/api/quotes?ts=${Date.now()}`),
      fetch(`${API_URL}/api/fixpoints`)
    ]);

    if (!qRes.ok || !fRes.ok) return;

    const quotesData = await qRes.json();
    const fixpointsData = await fRes.json();

    setItems([...quotesData]);      // forza refresh stato
    setFixpoints([...fixpointsData]);

  } catch (err) {
    console.error('Errore load preventivi:', err);
  }
};
...
useEffect(() => {
 ...
 const assignFixpoint = async (quoteId: number, fixpointId: number) => {
  ...
  await load();   // 🔥 RICARICA DAL BACKEND
