...
export default function PricesPage() {
  ...
  useEffect(()=>{
    fetch('http://localhost:3001/api/device-types').then(r=>r.json()).then(setDeviceTypes);
    fetch('http://localhost:3001/api/brands').then(r=>r.json()).then(setBrands);
  },[]);

  useEffect(()=>{
    fetch(`http://localhost:3001/api/repairs?device_type_id=${deviceTypeId ?? ''}`)
      .then(r=>r.json())
      .then(setRepairs);
  },[deviceTypeId]);
  ...
  useEffect(()=>{
    ...
    fetch(`http://localhost:3001/api/models?device_type_id=${deviceTypeId}&brand_id=${brandId}`)
      .then(r=>r.json())
      .then(data=>{
        setModels(data);
  ...
  const reloadPrices = async(id:number)=>{
    const res = await fetch(`http://localhost:3001/api/model-repairs?model_id=${id}`);
  ...
  const savePrice = async(repairId:number)=>{
    ...
    await fetch('http://localhost:3001/api/model-repairs',{
      method:'POST',
  ...
  const deleteRow = async(rowId:number)=>{
    ...
    await fetch(`http://localhost:3001/api/model-repairs/${rowId}`,{
      method:'DELETE'
    });
