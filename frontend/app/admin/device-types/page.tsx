...
export default function DeviceTypesPage() {
  ...
  // carica lista + usage
  const load = async () => {
    const res = await fetch('http://localhost:3001/api/device-types');
    const data: DeviceType[] = await res.json();
    setItems(data);

    // controlla quali sono usati
    const usage: Record<number, boolean> = {};

    await Promise.all(
      data.map(async (d) => {
        const r = await fetch(
          `http://localhost:3001/api/device-types/${d.id}/used`
        );
        const info = await r.json();
        usage[d.id] = info.used;
      })
    );
  ...
  const saveEdit = async () => {
    ...
    await fetch(`http://localhost:3001/api/device-types/${editingId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`http://localhost:3001/api/device-types/${id}`, {
      method: 'DELETE',
    });
