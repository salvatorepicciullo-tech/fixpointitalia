...
export default function ValuationsConfigPage() {
  ...
  useEffect(() => {
    ...
    const loadBase = async () => {
      ...
      if (Array.isArray(dt)) setDeviceTypes(dt);

      const br = await apiFetch('/api/brands');
      if (Array.isArray(br)) setBrands(br);

      const cfg = await apiFetch('/api/admin/valuation-configs');
      if (Array.isArray(cfg)) setConfigs(cfg);
