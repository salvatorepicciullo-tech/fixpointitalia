...
export default function RepairsPage() {
  ...
  const load = async () => {
    ...
    let url = 'http://localhost:3001/api/repairs';
...
const add = async () => {
  ...
  await fetch('http://localhost:3001/api/repairs', {
    method: 'POST',
  ...
  const saveEdit = async () => {
    ...
    await fetch(`http://localhost:3001/api/repairs/${editingId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`http://localhost:3001/api/repairs/${id}`, {
      method: 'DELETE',
    });
