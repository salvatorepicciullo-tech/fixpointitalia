...
export default function ModelsPage() {
  ...
  const loadModels = async (dtId: number, bId: number) => {
    const res = await fetch(
      `http://localhost:3001/api/models?device_type_id=${dtId}&brand_id=${bId}`
    );
  ...
  const addModel = async () => {
    ...
    await fetch('http://localhost:3001/api/models', {
      method: 'POST',
...
if (deviceTypeId && brandId) {
  ...
  const saveEdit = async () => {
    ...
    await fetch(`http://localhost:3001/api/models/${editingId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  const remove = async (id: number) => {
    ...
    await fetch(`http://localhost:3001/api/models/${id}`, {
      method: 'DELETE',
    });
