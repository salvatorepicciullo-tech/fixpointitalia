...
export default function ModelValuationPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/api/admin/models/${modelId}/base-value`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then(r => r.json()),

      fetch(`${API_URL}/api/admin/defects`, {
        headers: { Authorization: `Bearer ${token}` },
      }).then(r => r.json()),
    ])
  ...
  const saveBaseValue = async () => {
    ...
    await fetch(`${API_URL}/api/admin/models/${modelId}/base-value`, {
      method: 'POST',
      headers: {
  ...
  const savePenalty = async (defectId: number, penalty: number) => {
    ...
    await fetch(`${API_URL}/api/admin/models/${modelId}/defects`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer ${token}`,
