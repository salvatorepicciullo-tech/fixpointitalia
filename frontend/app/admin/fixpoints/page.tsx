...
export default function FixPointsPage() {
  ...
  const fetchFixPoints = async () => {
    ...
    const res = await fetch(
      `http://localhost:3001/api/admin/fixpoints${qs}`
    );
  ...
  const createFixPoint = async () => {
    const res = await fetch('http://localhost:3001/api/admin/fixpoints', {
      method: 'POST',
  ...
  const updateFixPoint = async () => {
    const res = await fetch(
      `http://localhost:3001/api/fixpoints/${form.id}`,
      {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(form),
  ...
  const toggleFixPoint = async (fp: FixPoint) => {
    ...
    await fetch(
      `http://localhost:3001/api/fixpoints/${fp.id}/${action}`,
      { method: 'PUT' }
    );
    fetchFixPoints();
  };
  ...
  const resetPassword = async () => {
    ...
    const res = await fetch(
      `http://localhost:3001/api/admin/users/${form.user_id}/reset-password`,
      { method: 'POST' }
    );
