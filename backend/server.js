...
function generatePracticeCode(prefix, id) {
...
let formattedDate = 'Non specificato';
...
if (preferred_datetime) {
  ...
  formattedDate = `${giorno}/${mese}/${anno} ore ${ore}:${minuti}`;
}

  const mapsLink = fp.address
    ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(fp.address + ' ' + fp.city)}`
    : '#';

  const whatsappLink = fp.phone
    ? `https://wa.me/${fp.phone.replace(/\D/g,'')}`
    : '#';

  return transporter.sendMail({
    from: `FixPoint Italia <${process.env.MAIL_USER}>`,
   to,
cc: process.env.MAIL_USER,
    ...
    <div style="margin:0;padding:0;background:#f4f6f9;font-family:Arial,sans-serif;">
      ...
      <div style="max-width:650px;margin:30px auto;background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 6px 18px rgba(0,0,0,0.08);">

        <div style="background:#111827;padding:25px;text-align:center;">
          <h2 style="color:white;margin:0;">FixPoint Italia</h2>
        </div>

        <div style="padding:35px;">

          <h2 style="margin-top:0;color:#111827;text-transform:capitalize;">
            ${type} Confermato
          </h2>

          <p>Ciao <strong>${customer_name || ''}</strong>,</p>

          <p>La tua richiesta è stata assegnata al centro assistenza:</p>

          <div style="background:#f9fafb;padding:20px;border-radius:10px;margin:20px 0;border:1px solid #eee;">
            <h3 style="margin:0 0 10px 0;">${fp.name || ''}</h3>
            <p>📍 ${fp.address || ''}, ${fp.city || ''}</p>
            <p>📞 ${fp.phone || '-'}</p>
            <p>📧 ${fp.email || '-'}</p>
          </div>
          ...
          <div style="margin:20px 0;text-align:center;">
            <a href="${whatsappLink}"
               style="display:inline-block;background:#25D366;color:white;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:bold;margin:5px;">
              WhatsApp
            </a>

            <a href="${mapsLink}"
               style="display:inline-block;background:#1a73e8;color:white;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:bold;margin:5px;">
              Google Maps
            </a>
          </div>
          ...
          <table style="width:100%;border-collapse:collapse;margin-top:15px;">
            <tr>
              <td>ID pratica</td>
              <td style="text-align:right;"><strong>#${practice_id}</strong></td>
            </tr>
  ...
  <tr>
    ...
    <td style="text-align:right;">€ ${max_value || 0}</td>
  </tr>
  <tr>
    <td>Penalità</td>
    <td style="text-align:right;">- € ${total_penalty || 0}</td>
  </tr>
  <tr>
    <td><strong>Valore finale</strong></td>
    <td style="text-align:right;font-size:18px;">
      <strong>€ ${final_value || 0}</strong>
    </td>
  </tr>
  ${
    defects
  ...
  `
          ...
          <p style="margin-top:25px;color:#555;font-size:14px;">
            Verrai contattato dal centro assistenza per conferma.
          </p>

        </div>

        <div style="background:#111827;color:white;text-align:center;padding:15px;font-size:12px;">
          © ${new Date().getFullYear()} FixPoint Italia
        </div>
...
/* =========================
   CHECK DEVICE TYPE USED
========================= */
app.get('/api/device-types/:id/used', (req, res) => {
  const { id } = req.params;

  db.get(
    'SELECT id FROM models WHERE device_type_id = ? LIMIT 1',
    [id],
    (err, row) => {
      if (err) {
        console.error(err);
...
console.log('🔥 VALUTAZIONE CHIAMATA');
  const {
    model_id,
    city,
    customer_name,
  ...
  [
    ...
    req.body.preferred_datetime || null,
  ],
  function (err) {
        ...
        if (Array.isArray(defect_ids) && defect_ids.length) {
         ...
         stmt.finalize(() => {
  sendValuationEmail(valuationId, fixpointIdFinal);
});
...
function sendValuationEmail(valuationId, fixpointIdFinal) {
  ...
  db.get(
    ...
    (errVal, valuation) => {
      ...
      if (errVal || !valuation) {
        console.error('Errore recupero valutazione:', errVal);
        return res.status(500).json({ error: 'Errore recupero valutazione' });
      }
      ...
      db.get(
        ...
        `,
        [fixpointIdFinal],
        async (errFix, fixpoint) => {

          const fp = fixpoint || {};
          const practiceCode = generatePracticeCode("V", valuationId);

          try {

            if (valuation.customer_email) {

              await sendFixpointEmail({
                to: valuation.customer_email,
                subject: `Valutazione ricevuta ${practiceCode}`,
                customer_name: valuation.customer_name,
                fixpoint: fp,
                practice_id: practiceCode,
                preferred_datetime: valuation.preferred_datetime,
                type: "valutazione",
                model: valuation.model,
                max_value: valuation.max_value,
                total_penalty: valuation.total_penalty,
                final_value: Math.max(0, valuation.max_value - valuation.total_penalty),
                defects: valuation.defects
              });

            }

          } catch (e) {
            console.log('Errore invio email valutazione:', e.message);
          }
...
(
  ...
  customer_phone || '',
  description || '',
  preferred_datetime || null   // 👈 AGGIUNTO
]
,
      function(err) {
...
function saveQuote(calculatedPrice) {
  ...
  db.run(
    ...
    (model_id, fixpoint_id, price,
     city, customer_name, customer_email, customer_phone, preferred_datetime, status)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'ASSIGNED')
    `,
    [
      model_id,
...
try {
  ...
  await sendFixpointEmail({
    to: customer_email,
    subject: `Preventivo confermato #${quoteId}`,
    customer_name,
    fixpoint: fp,
    practice_id: quoteId,
  ...
  console.log('Errore invio email:', e.message);
}
...
app.get('/api/quotes', (req, res) => {
  db.all(
    ...
    SELECT
      ...
      f.name AS fixpoint_name,   -- 🔥 AGGIUNTO
      f.city AS fixpoint_city,   -- 🔥 AGGIUNTO
    ...
    LEFT JOIN fixpoints f ON f.id = q.fixpoint_id   -- 🔥 JOIN FIXPOINT
...
/* =======================
   ADMIN – SAVE DEFECT PENALTIES
======================= */
app.post('/api/admin/models/:id/defect-penalties', auth, (req, res) => {
  if (req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Accesso negato' });
  }

  const modelId = req.params.id;
  const penalties = Array.isArray(req.body)
  ? req.body
  : (req.body.penalties || []);
  ...
  db.serialize(() => {
    ...
    const stmt = db.prepare(
      'INSERT INTO device_defect_penalties (model_id, defect_id, penalty) VALUES (?, ?, ?)'
    );

    penalties.forEach(p => {
      stmt.run(modelId, p.defect_id, p.penalty || 0);
    });
...
/* =======================
   ADMIN – DELETE SINGLE PENALTY
======================= */
app.delete('/api/admin/models/:modelId/defect-penalties/:defectId', auth, (req, res) => {
  if (req.user.role !== 'admin') {
    return res.status(403).json({ error: 'Accesso negato' });
  }

  db.run(
    'DELETE FROM device_defect_penalties WHERE model_id=? AND defect_id=?',
    [req.params.modelId, req.params.defectId],
    function (err) {
      if (err) return res.status(500).json({ error: 'Errore DB' });
      res.json({ success: true });
    }
